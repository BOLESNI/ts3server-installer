...
if [ "${OSNAME}" = "debian" ]; then
	...
	if [ "x$(lsb_release -s -c)" = "xxenial" ] || [ "x$(lsb_release -s -c)" = "xstretch" ] || [ "x$(lsb_release -s -c)" = "xbionic" ]  || [ "x$(lsb_release -s -c)" = "xbuster" ] ; then
	...
	yum -y install epel-release || yum -y install epel-release || yum -y install epel-release
	...
	pkglist="openssl sqlite wget httpd php bzip2 which anacron"
...
wget --no-check-certificate https://github.com/DEFNOX/ts3-cp/archive/master.tar.gz -O ts3cp.tar.gz
